...
def make_headers(token, profile_id=None):
    headers = {
...
def api_get(token, path, params=None, profile_id=None):
    url = f"{BASE_URL}{path}"
...
def api_post(token, path, data, profile_id=None):
    url = f"{BASE_URL}{path}"
    resp = requests.post(url, headers=make_headers(token, profile_id), json=data, timeout=30)
    return resp
...
def get_period_id(token, person_id, profile_id=None):
    """Получает period_id для ученика."""
    try:
        periods = api_get(token, "/api/soft_skills/v1/periods",
                          {"student_person_id": person_id}, profile_id=profile_id)
...
def check_already_filled(token, person_id, period_id, profile_id=None):
    """Проверяет, заполнена ли уже анкета для ученика."""
    try:
        resp = requests.get(
            f"{BASE_URL}/api/soft_skills/v1/survey/teacherSurvey",
            headers=make_headers(token, profile_id),
...
def main():
    ...
    # 1. Получаем токен и Profile-Id
    token = input("\n🔑 Введите Bearer токен (из DevTools → Network):\n  ").strip()
    if not token:
        print("Токен не может быть пустым.")
        sys.exit(1)

    profile_id = input("\n🆔 Введите Profile-Id (из DevTools → Request Headers):\n  ").strip()
    if not profile_id:
        print("Profile-Id не может быть пустым. Найди его в заголовках запроса (DevTools → Network → любой запрос → Request Headers → Profile-Id).")
        sys.exit(1)
    ...
    try:
        groups_data = api_get(token, "/api/ej/plan/teacher/v1/groups",
                              {"academic_year_id": 13}, profile_id=profile_id)
    ...
    # Группируем по предметам + собираем subject_id для каждой группы
    ...
    for g in all_groups:
        ...
        subjects[subj].append((gid, name))
        sid = g.get("subject_id")
        if sid:
            group_to_subject[gid] = sid
    ...
    for gid, gname in selected_groups:
        ...
        # Получаем оценки (с subject_id, если известен — как в браузере)
        ...
        if gid in group_to_subject:
            marks_params["subject_id"] = group_to_subject[gid]
        try:
            marks = api_get(token, "/api/ej/core/teacher/v1/marks", marks_params, profile_id=profile_id)
        ...
        # Получаем period_id
        period_id = get_period_id(token, valid[0]["person_id"], profile_id=profile_id)
        if not period_id:
        ...
        for s in valid:
            ...
            if check_already_filled(token, s["person_id"], period_id, profile_id=profile_id):
                skipped += 1
